...
tikz(file_geom, height = height, width = width, standAlone = standalone)
par(mar = rep(0, 4))
plot.new()
# if (FALSE) {
...
dev.off()
if (compile) {
  knitr::knit2pdf(file_geom, output = "geometry/ortho.tex")
}
## ---- obliq-1 ----
file_geom <- tempfile(fileext = ".tex")
tikz(file_geom, height = height, width = width, standAlone = standalone)
...
dev.off()
if (compile) {
  knitr::knit2pdf(file_geom, output = "geometry/obliq-1.tex")
}
## ---- obliq-2 ----
...
dev.off()
if (compile) {
  knitr::knit2pdf(file_geom, output = "geometry/obliq-2.tex")
}
## ---- obliq-3 ----
...
if (compile) {
  knitr::knit2pdf(file_geom, output = "geometry/obliq-3.tex")
}
