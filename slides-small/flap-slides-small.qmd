...
df_hist <- bind_rows(syd_mel, aus) %>% 
  mutate(scale = factor(scale, levels = c("Regions", "Australia")))
df_hist %>%
  ggplot() +
  ...
  ggtitle("Number of nights spent by Australian visitors") +
    theme(
...
2. Obtain forecasts $\hat{\bm{z}}_{t+h} = [\hat{\bm{y}}_{t+h}',\hat{\bm{c}}_{t+h}']'$
