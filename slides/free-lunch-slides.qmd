...
A model-independent post-forecast adjustment method that can reduce forecast variance.
...
If the base forecasts are unbiased, then the FLAP forecasts are also unbiased.
\end{alertblock}

### Projection matrix

1. The matrix $\bm{M}$ is a projection onto the space where the constraint $\bm{C}\bm{z}_t=\bm{0}$ is satisfied.
...
* $\bm{w}_{c_1y,h}$ is the forecast covariance between the first component and the original series.
...
1. The forecast variance is __reduced__ with FLAP
