...
$$ {#eq-mint}
where
$\bm{S} = \begin{bmatrix}\bm{I}_m \\\bm{\Phi}\end{bmatrix}$ contains the constraints, so that $\bm{y}_{t} = \bm{S}\bm{z}_{t}$, and
$$
...
\E(\hat{\bm{y}}_{t+h}|\mathcal{I}_t) = \E(\bm{y}_{t+h}|\mathcal{I}_t),
$$
then the projected forecasts defined in @eq-stmap are also unbiased:
$$
