---
title: "Free Lunch in Multivariate Forecasting: Reducing Variance Using Synthetic Linear Combination Forecast Projection"
...
abstract: "Abstact to be written."
pdf-engine: pdflatex
...
This article considers improving the accuracy of high-dimensional multivariate time series forecasting in a modern setting. There is much room for improvement with traditional univariate models overlooking information shared between different series and multivariate models relying on large numbers of observations to estimate. We propose to forecast linear combinations, which we call components, of the target collection of time series, and project them along with the forecasts of original series, which we call base forecasts, in a way such that the constraints imposed by the construction of components are satisfied. We show that this forecast projection method has significant gains over the base forecasts both theoretically and empirically. As a post-forecasting procedure, it is implemented for a given set of base forecasts, regardless of what models generate them. This allows the forecast projection to reduce forecast variance on top of practically any forecast model. Relatively simple univariate models can also have desirable performance as the projection can utilise information that is shared across series and captured in the components.

Our method has close connections to forecast reconciliation in the hierarchical forecasting literature. See @AthEtAl2023a for a review. In particular, the projection formulation is inspired by the minimum trace [MinT, @WicEtAl2019] solution of the forecast reconciliation problem. Forecast reconciliation is a method to modify forecasts using projection so that they conform to a specific hierarchical, grouped or temporal structure. Notably, @WicEtAl2019, @AthEtAl2017, and  @DiGir2023 have shown that forecast reconciliation can reduce forecast variance theoretically and empirically, in cross-sectional, temporal and both cross-sectional and temporal settings respectively. @PanEtAl2021 have provided insight into the geometric interpretation of the projection used in forecast reconciliation. However, forecast reconciliation cannot be directly applied in a general multivariate time series setting as a natural structure in the data is required. On the other hand, if the structure of the series is already followed by the forecasts, sensible forecast reconciliation will not change the forecast, and therefore no gain in forecast accuracy is achieved. Our method differs from forecast reconciliation in the sense that it can be applied to any general multivariate forecasting problem without an inherent structure. When a structure does exist, the principles of the proposed projection method can still be applied on top of forecast reconciliation to achieve desirable forecast variance reduction.

One strand of literature attempting to reduce variance is bootstrap aggregation [bagging, @Bre1996]. 
In bagging, the final prediction is produced as an ensemble (e.g. average) of predictions made by predictors built on bootstrapped data. Bagging can reduce prediction variance without increasing bias [@HasEtAl2003], and it does so primarily by mitigating model uncertainty [@PetEtAl2018], or what we might call model misspecification error.
One of the early works of using bagging to improve forecast accuracy in a time series context is @BerEtAl2016, where the authors apply bagging on exponential smoothing.
Similarly to bagging, syntactic data forecast projection can also reduce forecast variance by reducing model misspecification error, as can be seen in the simulation example in @sec-simulation.
We highlight two distinctions between forecast projection and bagging.
Firstly, bagging involves producing random draws from a sample of possibly univariate series, whereas our method generates linear combinations of multiple series.
Secondly, bagging is model dependent: it is a procedure applied to enhance the model or the system of models that produce the forecasts, where the same model or the system of models are fitted repeatedly. Our method is model independent - it linearly transforms given forecasts, regardless of which models they come from. As a result, the two methods can be used in conjunction: the projection can be applied to forecasts produced by a bagged predictor.


Another broader strand of literature concerning the improvement of forecast accuracy is forecast combination. If we consider the narrow sense of point forecast combination, it is defined as combining multiple forecasts from different sources (models) of the same target series. See @WanEtAl2023 for a review. Our forecast projection method differs from forecast combination mainly in the forecasts we combine (project). Any projected forecast of a particular series is a combination of forecasts of all series in the collection and their components, not only of the same target series. Another difference lies in the sources of forecasts. The forecasts in forecast combination are often generated by different models, possibly across classes. On the other hand, while forecast projection is model independent and allows different models to be used, it is easier to simply use a single class of models with an automatic fitting procedure when building base forecasts on different series and components. With the focus of forecast combinations mainly on univariate forecasts, like bagging, forecast combination and forecast projection can be used naturally in a sequential order: the combined forecasts of each series and potentially the component can be used as base forecasts to be projected. 

If we consider the broad sense of forecast combination that includes any procedure where forecasts are combined, the aforementioned forecast reconciliation, bagging and our proposed forecast projection can all be viewed as forms of forecast combination with different objects to be combined. @PetSpi2021 overview a group of methods utilising combination techniques which they call "the wisdom of data" including bagging, theta method [@AssNik2000], temporal aggregation [@KouEtAl2014; @AthEtAl2017], forecasting with sub-seasonal series [FOSS, @LiEtAl2022c] and forecast combination with multiple starting points [@DisPet2015], differing in transformations or selections of series to forecast, forecasting models, and combination weights. Not unlike the wisdom of data techniques, forecast projection also aims at exploiting information in the data, with a focus on the shared information that can be captured by linear combinations of the series. 

Lastly, the use of components bears resemblance to the Dynamic Factor Model (DFM), specifically those used in a forecasting setting where the components (factors) are estimated using Principal Component Analysis (PCA) [@StoWat2002a; @StoWat2002; @StoWat2012], and their extension in the machine learning literature [@DeEtAl2019]. DFM operates on the assumption that the multivariate time series possesses common components and their dynamics are governed by the dynamics of these components, typically assumed to follow a Vector AutoRegressive (VAR) model. Our method differs in principle as a post-forecasting adjustment without any assumption on the parametric form of the dynamic. Even though both DFM and forecast projection use components, the forecast from DFM can be further improved as base forecasts in the projection. This is demonstrated in @sec-simulation and @sec-empirical-applications, where we also show how the performance of projected forecasts from univariate ARIMA models is comparable to base DFM forecasts, with the help of components.

The rest of the paper is structured as follows. In @sec-method, we propose the forecast projection method with its theoretic properties and estimation methods. In @sec-simulation, we present a simulation example demonstrating its performance and implications on sources of uncertainty. @sec-empirical-applications examines the performance of forecast projection in two empirical applications: forecasting Australian domestic tourism and forecasting macroeconomic variables in the FRED-MD data set. @sec-conclusion concludes.

# Method {#sec-method}

Let $\bm{z}_t\in\mathbf{R}^m$ be a vector of $m$ series at time $t$. Let $\bm{y}_{t} = \begin{bmatrix}\bm{z}_t\\ \bm{c}_{t}\end{bmatrix}$ be the collection of series $\bm{z}_t$ and components $\bm{c}_{t}$ where $\bm{c}_{t} = \bm{\Phi}\bm{z}_t\in \mathbf{R}^p$. Let $\hat{\bm{y}}_{t+h}$ be the $h$-step-ahead base forecast of $\bm{y}_{t}$. The forecast variance covariance matrix is 
$$
\operatorname{Var}(\hat{\bm{y}}_{t+h}-\bm{y}_{t+h}) = \bm{W}_h.
$$
We project the base forecast into the space where the constraints are imposed: 
$$
...
$$ {#eq-M}
where $\tilde{\bm{y}}_{t+h}$ is the projected (constrained) forecast applying the component constraints 
$$
\bm{C} = \begin{bmatrix} -\underset{p \times m}{\bm{\Phi}}& \underset{p \times p}{\bm{I}_{p}} \end{bmatrix},
$$
such that $\bm{C}\bm{y}_t= \bm{c}_{t} - \bm{\Phi}\bm{z}_t = \bm{0}$ for any $t$, and $\bm{I}_p$ is the identity matrix of dimension $p$.
Note $\bm{M}$ is a projection matrix such that $\bm{M}\bm{M} = \bm{M}$.
Denote $\hat{\bm{z}}_{t+h}$ and $\tilde{\bm{z}}_{t+h}$ as the first $m$ elements of $\hat{\bm{y}}_{t+h}$ and $\tilde{\bm{y}}_{t+h}$, respectively the base and projected forecasts of $\bm{z}_t$, and $\hat{\bm{c}}_{t+h}$ and $\tilde{\bm{c}}_{t+h}$ similarly. Denote $\bm{J}_{i} = \begin{bmatrix}\bm{I}_m& \bm{0}_{m\times i}\end{bmatrix}$ as a selection matrix that can pick out the first $m$ element of a vector or the first $m$ rows of a matrix, and denote $\bm{J}=\bm{J}_p$ for simplicity, then the constrained projected forecast of $\bm{z}_t$ can be found by 
$$
\tilde{\bm{z}}_{t+h} = \bm{J}\tilde{\bm{y}}_{t+h} = \bm{J}\bm{M} \hat{\bm{y}}_{t+h}.
$$ {#eq-lcmap}
We are ready to present a few immediate results. The proofs are all given in the Appendix.

::: {#lem-coherent}

The projected forecast $\tilde{\bm{y}}_{t+h}$ satisfies the constraint such that
$$
...
$$
\operatorname{Var}(\tilde{\bm{y}}_{t+h} - \bm{y}_{t+h}) = \bm{M}\bm{W}_h\bm{M}' = \bm{M}\bm{W}_h,
$$
and the forecast variance covariance matrix of the projected $h$-step-ahead forecasts $\tilde{\bm{z}}_{t+h}$ is 
$$
\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h}) = \bm{J}\bm{M}\bm{W}_h\bm{J}'.
$$
:::
...
&= \operatorname{Var}(\bm{M}\hat{\bm{y}}_{t+h} - \bm{M}\bm{y}_{t+h}) \\
&= \bm{M}\operatorname{Var}(\hat{\bm{y}}_{t+h} - \bm{y}_{t+h})\bm{M}' \\
&= \bm{M}\bm{W}_h\bm{M}'.
\end{aligned}
$$
...
&+  \bm{W}_h\bm{C}'(\bm{C}\bm{W}_h\bm{C}')^{-1}\bm{C}\bm{W}_h\bm{C}'(\bm{C}\bm{W}_h\bm{C}')^{-1}\bm{C}\bm{W}_h \\
=& \bm{W}_h - \bm{W}_h\bm{C}'(\bm{C}\bm{W}_h\bm{C}')^{-1}\bm{C}\bm{W}_h \\
=& (\bm{I} - \bm{W}_h\bm{C}'(\bm{C}\bm{W}_h\bm{C}')^{-1}\bm{C})\bm{W}_h \\
=& \bm{M}\bm{W}_h.
\end{aligned}
$$
To get $\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h})$, we just need to recognise it is the first $m\times m$ leading principal submatrix of $\operatorname{Var}(\tilde{\bm{y}}_{t+h} - \bm{y}_{t+h})$.
...
$$
\begin{aligned}
\operatorname{Var}(\hat{\bm{y}}_{t+h} - \bm{y}_{t+h}) -\operatorname{Var}(\tilde{\bm{y}}_{t+h} - \bm{y}_{t+h}) &=\bm{W}_h-\bm{M}\bm{W}_h\\
...
$$
\operatorname{Var}(\hat{\bm{z}}_{t+h} - \bm{z}_{t+h}) -\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h}) = \bm{J}\bm{W}_h\bm{C}'(\bm{C}\bm{W}_h\bm{C}')^{-1}\bm{C}\bm{W}_h\bm{J}'
$$
...
\end{aligned}
$$
We only focus on the forecast variance of the original series, which is $\operatorname{tr}(\operatorname{Var}(\hat{\bm{z}}_{t+h} - \bm{z}_{t+h}) -\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h})) = \frac{1}{2}\operatorname{tr}(\bm{\Phi}'\bm{\Phi})$.
When $p<m$, since $\bm{\Phi}'\bm{\Phi}$ is idempotent (i.e. $(\bm{\Phi}'\bm{\Phi})(\bm{\Phi}'\bm{\Phi})= \bm{\Phi}'\bm{\Phi}\bm{\Phi}'\bm{\Phi} = \bm{\Phi}'\bm{\Phi}$), we have $\operatorname{tr}(\bm{\Phi}'\bm{\Phi}) = \operatorname{rank}(\bm{\Phi}'\bm{\Phi}) = p$. The reduction on the forecast variance of the original series is $p/2$. 
When $p=m$, we have $\operatorname{tr}(\bm{\Phi}'\bm{\Phi}) = \operatorname{tr}(\bm{I}_m) = m$, and the reduction is $m/2$.
...
$$
\operatorname{tr}(\operatorname{Var}(\hat{\bm{z}}_{t+h} - \bm{z}_{t+h}) -\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h})) = \operatorname{tr}(\bm{J}\bm{W}_h\bm{C}'(\bm{C}\bm{W}_h\bm{C}')^{-1}\bm{C}\bm{W}_h\bm{J}')
$$ {#eq-trdvar}
...
Suppose now we want to include $q$ more components $\bm{c}_{t}^*=\bm{\Phi}^*\bm{z}_t$ in the reconciliation. We define $\bm{y}_{t}^* = \begin{bmatrix}\bm{y}_{t}\\ \bm{c}_{t}^*\end{bmatrix}$, the constraint matrix
$$
\bm{C}^* =
\begin{bmatrix} 
...
\end{bmatrix}
$$ {#eq-Cstar}
where $\overline{\bm{C}}$ is the first $p$ rows of $\bm{C}^*$ and $\underline{\bm{C}}$ is the rest $q$ rows of $\bm{C}^*$,
the forecast variance covariance matrix 
$$
\operatorname{Var}(\hat{\bm{y}}_{t+h}^* - \bm{y}_{t+h}^*) = \bm{W}_h^* = \begin{bmatrix}\bm{W}_h & \bm{W}_{yc, h}^*\\ \bm{W}_{cy, h}^* & \bm{W}_{c,h}^*\end{bmatrix}.
$$
...
$$
\begin{aligned}
\bm{W}_h^*\overline{\bm{C}}' = & \begin{bmatrix}\bm{W}_h & \bm{W}_{yc, h}^*\\ \bm{W}_{cy, h}^* & \bm{W}_{c}^*\end{bmatrix}
\begin{bmatrix} 
...
\end{aligned}
$$
If we only consider the forecast performance relevant to $\bm{z}_{t+h}$, and define $\bm{J}^* = \bm{J}_{p+q}= \begin{bmatrix}\bm{I}_{m} & \bm{0}_{m\times(p+q)} \end{bmatrix}$, we have
$$
\tilde{\bm{z}}_{t+h}^+ = \bm{J}^*\tilde{\bm{y}}_{t+h}^+ = \bm{J}\tilde{\bm{y}}_{t+h} = \tilde{\bm{z}}_{t+h}.
$$
This means adding additional components without imposing the corresponding constraints will yield the same projected forecasts as if these additional components are not added, which implies the forecast variance stays the same:
$$
\operatorname{Var}(\tilde{\bm{z}}_{t+h}^+ - \bm{z}_{t+h}) = \operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h}) = \bm{J}\bm{M}\bm{W}_h\bm{J}'.
$$ {#eq-acnc}
...
$$
\operatorname{Var}(\tilde{\bm{y}}_{t+h}^+ - \bm{y}_{t+h}^*) = \bm{M}^{+}\bm{W}_h^* = (\bm{I}_{m+p+q} -\bm{W}_h^*\overline{\bm{C}}'(\overline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}')^{-1}\overline{\bm{C}})\bm{W}_h^*
$$
and
$$
\operatorname{Var}(\tilde{\bm{y}}_{t+h}^* - \bm{y}_{t+h}^*) = \bm{M}^*\bm{W}_h^* = (\bm{I}_{m+p+q} - \bm{W}_h^*\bm{C}^{*\prime}(\bm{C}^*\bm{W}_h^*\bm{C}^{*\prime})^{-1}\bm{C}^*)\bm{W}_h^*.
$$
Taking the difference, we have
$$
\begin{aligned}
\operatorname{Var}&(\tilde{\bm{y}}_{t+h}^+ - \bm{y}_{t+h}^*) - \operatorname{Var}(\tilde{\bm{y}}_{t+h}^* - \bm{y}_{t+h}^*) \\
=& (\bm{W}_h^*\bm{C}^{*\prime}(\bm{C}^*\bm{W}_h^*\bm{C}^{*\prime})^{-1}\bm{C}^* -\bm{W}_h^*\overline{\bm{C}}'(\overline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}')^{-1}\overline{\bm{C}})\bm{W}_h^* \\
=& \bm{W}_h^*(\bm{C}^{*\prime}(\bm{C}^*\bm{W}_h^*\bm{C}^{*\prime})^{-1}\bm{C}^* -\overline{\bm{C}}'(\overline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}')^{-1}\overline{\bm{C}})\bm{W}_h^*.
\end{aligned}
$$
...
(\overline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}')^{-1} + 
(\overline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}')^{-1}\overline{\bm{C}}\bm{W}_h^*\underline{\bm{C}}'(
\underline{\bm{C}}\bm{M}^{+}\bm{W}_h^*\underline{\bm{C}}'
)^{-1}
\underline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}'
(\overline{\bm{C}}\bm{W}_h^*\overline{\bm{C}}')^{-1},\\
b 
...
\operatorname{Var}(\tilde{\bm{z}}_{t+h}^* - \bm{z}_{t+h}) \\
&=
\bm{J}^*\operatorname{Var}(\tilde{\bm{y}}_{t+h}^+ - \bm{y}_{t+h}^*)\bm{J}^{*\prime} -
\bm{J}^*\operatorname{Var}(\tilde{\bm{y}}_{t+h}^* - \bm{y}_{t+h}^*)\bm{J}^{*\prime} \\
&=
\bm{J}^*
\bm{W}_h^*
...
$$
\begin{aligned}
&\operatorname{tr}(\operatorname{Var}(\hat{\bm{z}}_{t+h} - \bm{z}_{t+h}) -\operatorname{Var}(\tilde{\bm{z}}_{t+h}^* - \bm{z}_{t+h})) 
-
\operatorname{tr}(\operatorname{Var}(\hat{\bm{z}}_{t+h} - \bm{z}_{t+h}) -\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h})) \\
&=
\operatorname{tr}(
...
$$
\operatorname{tr}(\bm{J}_1\bm{W}_h^{(1)}\bm{\psi}_1'(\bm{\psi}_1\bm{W}_h^{(1)}\bm{\psi}_1')^{-1}\bm{\psi}_1\bm{W}_h^{(1)}\bm{J}_1') =
(\bm{\psi}_1\bm{W}_h^{(1)}\bm{\psi}_1')^{-1}\bm{\psi}_1\bm{W}_h^{(1)}\bm{J}_1'\bm{J}_1\bm{W}_h^{(1)}\bm{\psi}_1',
$$ {#eq-trdvar1}
where
$$
\begin{aligned}
\bm{\psi}_1\bm{W}_h^{(1)}\bm{J}_1' =& 
\begin{bmatrix}
-\bm{\phi}_1 & 1
\end{bmatrix} 
\begin{bmatrix}
...
@eq-trdvar1 is obviously non-negative. For it to be larger than $0$, we need $\bm{\psi}_1\bm{W}_h^{(1)}\bm{J}_1' \neq 0$, which gives $\bm{\phi}_1\bm{W}_{z,h} \neq \bm{W}_{c_1z,h}$.
...
\bm{\psi}_i'.
$$
Similar to before, we would want $\bm{\psi}_i\bm{M}^{+}_i\bm{W}_h^{(i)}\bm{J}_i' \neq \bm{0}$. Note that $\bm{\psi}_i$ conserns the first $m$ rows and the last row of $\bm{M}^{+}_i\bm{W}_h^{(i)}$, and $\bm{J}_i'$ concerns the first $m$ columns. Combined with the implication from @eq-acnc that the $m\times m$ leading principal submatrix in equation $\bm{J}_i\bm{M}^{+}_i\bm{W}_h^{(i)}\bm{J}_i'=\bm{J}_{i-1}\bm{M}_{i-1}\bm{W}_h^{(i-1)}\bm{J}_{i-1}'$ is the same, we suppress the straightforward yet tiresome details, and obtain 
$$
\bm{\phi}_i\bm{W}_{\tilde{z},h}^{(i-1)} \neq \begin{bmatrix}\bm{0}_{1\times m+i-1} & 1 \end{bmatrix}\bm{M}^{+}_i\bm{W}_h^{(i)}\bm{J}_i',
$$
where $\bm{W}_{\tilde{z},h}^{(i-1)}=\bm{J}_{i-1}\bm{M}_{i-1}\bm{W}_h^{(i-1)}\bm{J}_{i-1}'$ is the projected forecast variance of the original series using the first $i-1$ components, and the right hand side of the inequality is simply a one-row matrix consisting of the first $m$ elements in the last row of $\bm{M}^{+}_i\bm{W}_h^{(i)}$, which can be denoted as $\bm{W}_{\tilde{c}_i\tilde{z},h}^{(i-1)}$ and interpreted as the covariance between the projected forecast of the original series using the first $i-1$ components, and the projected forecast of the $i$th component using the first $i-1$ components. 
:::
:::

If we look at each element of the terms in @eq-pcon, we can interpret the condition in the following way: for at least one series, the forecast covariance between the new component and this series is not exactly a linear combination, defined by $\bm{\phi}_1$, of the forecast variance of this series and the forecast covariance between this series and all other series. For the $i$-component in @eq-pconi, the interpretation is similar, where the linear combination is defined by $\bm{\phi}_i$ and the forecast variance is replaced by the projected forecast variance and covariance associated with the first $i-1$ components. This interpretation constitutes a measure of forecast information. For the new component to be beneficial, the information brought by this new component, measured as the covariance, cannot be a combination of already existing information.
...
$$
\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h}) = \bm{G}\bm{W}_h\bm{G}'.
$$
...
\text{ and }\tilde{\bm{e}}_{y,t+h} &= \bm{y}_{t+h} - \tilde{\bm{y}}_{t+h}
...
Left multiply $\bm{G}$ to both sides, we have
$$
\begin{aligned}
...
Thus, we have 
$$
\operatorname{Var}(\tilde{\bm{z}}_{t+h} - \bm{z}_{t+h}) =\operatorname{Var}(\tilde{\bm{e}}_{z,t+h})=\operatorname{Var}(\bm{G}\hat{\bm{e}}_{y,t+h})= \bm{G}\operatorname{Var}(\hat{\bm{e}}_{y,t+h})\bm{G}'= \bm{G}\bm{W}_h\bm{G}'.
$$
...
Recognising $\bm{G}$ is of dimension $m\times (m+p)$, this problem can be effectively splitted into independent subproblems such that 
$$
...
This can be proved in a few different ways. We adopt the approach by @AndNar2022 to obtain the solution to @eq-obj, but the procedure from @Lue1969 [p. 85] can also be used, where the problem is divided to @eq-subobj and reconstructed to find the solution to @eq-obj.
...
\operatorname{tr}(\bm{H}(2\bm{W}_h\bm{G}'-\bm{S}\bm{\Lambda}')) &=0 \\
...
\end{aligned}
$$
becasue $\bm{G}\bm{S} = \bm{I}$. Putting it back in, we have
$$
\begin{aligned}
...
## Component constuction {#sec-component-construction}
...
#| fig-cap: 'Average ranks of $1$-, $6$- and $12$-step-ahead cross-validation MSE of different model and component specifications on the visnights data. The methods using forecast projection are named as "{Model}-{Comp. Weights}-{No. Comp.}". The base models are named as "{Model}-Base" and these points are marked with triangles. The shaded region is the interval of the best performing model. Methods outside the shaded region are significantly worse than the best model.'
```

```{r fig-visnights-line}
#| fig-cap: "MSE of different component construction methods by the number of components $p$ used in forecast projection with ETS models on the visnights data, for forecast horizons $1$, $6$ and $12$. The vertical black line indicates the location of $p=m$ the number of series."
```
